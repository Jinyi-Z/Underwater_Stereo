...
class feature_extraction1(nn.Module):
    def __init__(self, concat_feature=False, concat_feature_channel=12):
        super(feature_extraction1, self).__init__()
        self.concat_feature = concat_feature
        self.inplanes = 32
        # in_channels, out_channels, kernel_size, stride, pad, dilation
        self.input_modify = nn.Sequential(convbn(6, 3, 3, 1, 1, 1),nn.ReLU(inplace=True))
...
class GwcNet_Concat(nn.Module):
    def __init__(self, maxdisp, use_concat_volume=False):
        super(GwcNet_Concat, self).__init__()
        ...
        if self.use_concat_volume:
            ...
            self.feature_extraction = feature_extraction1(concat_feature=True,
                                                         concat_feature_channel=self.concat_channels)
        else:
            self.concat_channels = 0
            self.feature_extraction = feature_extraction1(concat_feature=False)
    ...
    def forward(self, left, right):
        # left = torch.cat((left_water, left_air), dim=1)
        # right = torch.cat((right_water, right_air), dim=1)
...
def GwcNet_G_Concat(d):
    return GwcNet_Concat(d, use_concat_volume=False)

def GwcNet_GC_Concat(d):
    return GwcNet_Concat(d, use_concat_volume=True)
